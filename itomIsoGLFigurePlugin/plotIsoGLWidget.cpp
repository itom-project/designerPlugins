...
const char* dont_scale_units[] = {
    "frame", "frames", "frm", "frms", "digit", "digits", "Bild", "Bilder", "Wert", "-"};
...
plotGLWidget::plotGLWidget(
    QMenu* contextMenu, QGLFormat& fmt, QWidget* parent, const QGLWidget* shareWidget) :
    QGLWidget(fmt, parent, shareWidget, Qt::Widget),
    m_contextMenu(contextMenu), m_pParent(parent), m_paletteNum(0), m_lineplotUID(0),
    m_cBarTexture(0), m_drawTitle(false), m_backgnd(true), m_drawLightDir(false),
    m_forceCubicVoxel(false), m_forceReplot(true), m_isInit(0), m_gamma(1), m_TransX(0.0),
    m_TransY(0.0), m_TransZ(-2.0), m_colorBarMode(COLORBAR_NO), m_currentColor(1), m_fontsize(10),
    m_linewidth(1), m_ticklength(1.0), m_z_tickmulti(1.0), m_cmplxMode(0.0), m_cmplxState(false),
    m_stackState(false), m_zAmpl(1.0), m_elementMode(PAINT_TRIANG),

    // GL Plot vars
    m_NumElements(0), m_pTriangles(NULL), m_pColTriangles(NULL), m_pColIndices(NULL),
    m_pNormales(NULL), m_pPoints(NULL),

    // DataObject Vars
    m_colorMode(1), m_pContentDObj(NULL),
#ifdef USEPCL
    m_pContentPC(NULL), m_pContentPM(NULL),
#endif
...
void plotGLWidget::paintGL()
{
    ...
    if ((((m_pTriangles == NULL) && (m_elementMode == PAINT_TRIANG)) &&
         ((m_pPoints == NULL) && (m_elementMode == PAINT_POINTS))) ||
        (m_pColTriangles == NULL) || (m_NumElements == 0))
    {
        m_isInit &= ~IS_RENDERING;
        return;
    }
    ...
    if (m_colorMode == 0 && m_elementMode == PAINT_TRIANG)
    {
        ...
        norm = sqrt(
            (positionLS[0] * positionLS[0]) + (positionLS[1] * positionLS[1]) +
            (positionLS[2] * positionLS[2]));
        positionLS[0] /= norm * 1;
        positionLS[1] /= norm * 1;
...
inline ito::RetVal plotGLWidget::NormalizeObj(cv::Mat& scaledTopo, ito::float64& normedInvalid)
{
    ...
    if (!ito::isNotZero<ito::float64>(norm))
        norm = 1.0; // if is zero set to 1.0
...
//----------------------------------------------------------------------------------------------------------------------------------
template <>
inline ito::RetVal plotGLWidget::NormalizeObj<ito::complex64>(
    cv::Mat& scaledTopo, ito::float64& normedInvalid)
{
    ito::RetVal retVal(retOk);
    cv::Mat* topoMat = (cv::Mat*)(m_pContentWhileRastering->get_mdata()[0]);
...
//----------------------------------------------------------------------------------------------------------------------------------
template <>
inline ito::RetVal plotGLWidget::NormalizeObj<ito::complex128>(
    cv::Mat& scaledTopo, ito::float64& normedInvalid)
{
    ito::RetVal retVal(retOk);
    cv::Mat* topoMat = (cv::Mat*)(m_pContentWhileRastering->get_mdata()[0]);
...
ito::RetVal plotGLWidget::GLSetPointsPCL(void)
{
    ...
    {
        // If m_elementMode == PAINT_POINTS try to paint points for faster, less memory consuming
        // visualisation
        if (m_elementMode == PAINT_POINTS)
        {
            m_pColTriangles = static_cast<GLubyte*>(calloc(width * height * 4, sizeof(GLubyte)));
            m_pColIndices =
                static_cast<unsigned char*>(calloc(width * height, sizeof(unsigned char)));
            ...
            if (m_pColTriangles == NULL || m_pPoints == NULL || m_pColIndices == NULL)
            {
                ...
                m_NumElements = 0;
                retVal +=
                    ito::RetVal(ito::retError, 0, tr("Error allocating memory").toLatin1().data());
            }
        }
    }
    ...
    if (!retVal.containsError())
    {
        if (m_elementMode == PAINT_POINTS)
        {
            ...
            case ito::pclXYZ:
                ...
                pclFillPtBuf<pcl::PointXYZ>(
                    ppclXYZ, xscale, xshift, yscale, yshift, zscale, zshift);
                break;

            case ito::pclXYZI:
                ppclXYZI = m_pContentPC->toPointXYZI().get();
                pclFillPtBuf<pcl::PointXYZI>(
                    ppclXYZI, xscale, xshift, yscale, yshift, zscale, zshift);
                break;

            case ito::pclXYZINormal:
                ppclXYZINormal = m_pContentPC->toPointXYZINormal().get();
                pclFillPtBuf<pcl::PointXYZINormal>(
                    ppclXYZINormal, xscale, xshift, yscale, yshift, zscale, zshift);
                break;

            case ito::pclXYZNormal:
                ppclXYZNormal = m_pContentPC->toPointXYZNormal().get();
                pclFillPtBuf<pcl::PointNormal>(
                    ppclXYZNormal, xscale, xshift, yscale, yshift, zscale, zshift);
                break;

            case ito::pclXYZRGBA:
                ppclXYZRGBA = m_pContentPC->toPointXYZRGBA().get();
                pclFillPtBuf<pcl::PointXYZRGBA>(
                    ppclXYZRGBA, xscale, xshift, yscale, yshift, zscale, zshift);
                break;

            case ito::pclXYZRGBNormal:
                ppclXYZRGBNormal = m_pContentPC->toPointXYZRGBNormal().get();
                pclFillPtBuf<pcl::PointXYZRGBNormal>(
                    ppclXYZRGBNormal, xscale, xshift, yscale, yshift, zscale, zshift);
                break;

            default:
            case ito::pclInvalid:
                retVal +=
                    ito::RetVal(ito::retError, 0, tr("invalid point cloud").toLatin1().data());
                break;
            }
        }
    }
    ...
    if (m_NumElements != 0 || !retVal.containsError())
    {
        ...
        {
            m_pColIndices =
                (unsigned char*)realloc(m_pColIndices, m_NumElements * sizeof(unsigned char));
            m_pPoints = (GLfloat*)realloc(m_pPoints, m_NumElements * 3 * sizeof(GLfloat));
            m_pColTriangles =
                (unsigned char*)realloc(m_pColTriangles, m_NumElements * 4 * sizeof(GLubyte));
        }
        else
        {
            m_pColIndices = (unsigned char*)realloc(
                m_pColIndices, m_NumElements * 2 * 3 * sizeof(unsigned char));
            m_pTriangles = (GLfloat*)realloc(m_pTriangles, m_NumElements * 2 * 9 * sizeof(GLfloat));
            m_pColTriangles =
                (unsigned char*)realloc(m_pColTriangles, m_NumElements * 2 * 12 * sizeof(GLubyte));
    ...
    if (m_NumElements == 0 || retVal.containsError())
    {
        ...
        m_NumElements = 0;
        retVal += ito::RetVal(
            ito::retError, 0, tr("Error calculating points / triangles").toLatin1().data());
        m_isInit &= ~IS_CALCTRIANG;
    }
#endif
    return retVal;
...
ito::RetVal plotGLWidget::GLSetTriangles(void)
{
...
#endif
    {
        m_isInit &= ~HAS_TRIANG;
        return ito::RetVal(
            ito::retError, 0, tr("DataObject empty, calc triangles failed").toLatin1().data());
    }
    ...
    default:
        return ito::RetVal(
            ito::retError,
            0,
            tr("Unknown DataObject-Type, calc triangles failed").toLatin1().data());
    }
    ...
    if (!retVal.containsError())
    {
        ...
        {
            m_pColTriangles =
                static_cast<GLubyte*>(calloc(xsizeObj * ysizeObj * 4, sizeof(GLubyte)));
            m_pColIndices =
                static_cast<unsigned char*>(calloc(xsizeObj * ysizeObj, sizeof(unsigned char)));
        ...
        {
            m_pColTriangles =
                static_cast<GLubyte*>(calloc(xsizeObj * ysizeObj * 2 * 12, sizeof(GLubyte)));
            m_pColIndices = static_cast<unsigned char*>(
                calloc(xsizeObj * ysizeObj * 2 * 3, sizeof(unsigned char)));

            m_pTriangles =
                static_cast<GLfloat*>(calloc(xsizeObj * ysizeObj * 2 * 9, sizeof(GLfloat)));
            m_pNormales =
                static_cast<GLfloat*>(calloc(xsizeObj * ysizeObj * 2 * 9, sizeof(GLfloat)));

            if (m_pColTriangles == NULL || m_pTriangles == NULL || m_pColIndices == NULL ||
                m_pNormales == NULL)
            {
                m_isInit &= ~HAS_TRIANG;
                m_NumElements = 0;
...
#pragma omp for schedule(guided)
                ...
                for (cntY = 0; cntY < ysizeObj - 1; cntY++)
                {
                    ...
                    for (cntX = 0; cntX < xsizeObj - 1; cntX++)
                    {
                        ...
                        if ((fabs(color - dpixel1) < threshold) &&
                            ito::isFinite<ito::float64>(dpixel1) && (dpixel1 != invalidValue))
                        {
#if (USEOMP)
#pragma omp critical
                            {
                            ...
                            m_pPoints[count * 3 + 1] =
                                ((double)(cntY + 1) * m_windowYScale - yshift);
                            m_pPoints[count * 3 + 2] = zscale * dpixel1 - zshift;

                            m_pColIndices[count] =
                                cv::saturate_cast<unsigned char>(dpixel1 * 255.0);
                        }
                    }
                }
            }
                ...
                for (cntY = 0; cntY < ysizeObj - 1; cntY++)
                {
                    ...
                    for (cntX = 0; cntX < xsizeObj - 1; cntX++)
                    {
                        ...
                        if (ito::isFinite<ito::float64>(dpixel1) &&
                            ito::isFinite<ito::float64>(dpixel2) &&
                            ito::isFinite<ito::float64>(dpixel2))
                        {
                        ...
                        if ((fabs(color - dpixel1) < threshold) &&
                            (fabs(color - dpixel2) < threshold) &&
                            (fabs(color - dpixel3) < threshold) &&
                            /*(fabs(dpixel1) < 1.6e308)*/ isFinite && (dpixel1 != invalidValue) &&
                            (dpixel2 != invalidValue) && (dpixel3 != invalidValue))
                        {
#if (USEOMP)
#pragma omp critical
                            {
                            ...
                            m_pTriangles[count * 9 + 1] =
                                ((double)(cntY + 1) * m_windowYScale - yshift);
                            m_pTriangles[count * 9 + 2] = zscale * dpixel1 - zshift;

                            m_pTriangles[count * 9 + 3] =
                                ((double)(cntX + 1) * m_windowXScale - xshift);
                            ...
                            {
                                Vec1[n] =
                                    m_pTriangles[count * 9 + 3 + n] - m_pTriangles[count * 9 + n];
                                Vec2[n] =
                                    m_pTriangles[count * 9 + 6 + n] - m_pTriangles[count * 9 + n];
                            }

                            m_pNormales[count * 9 + 6] = m_pNormales[count * 9 + 3] =
                                m_pNormales[count * 9] = (Vec1[1] * Vec2[2] - Vec1[2] * Vec2[1]);
                            m_pNormales[count * 9 + 7] = m_pNormales[count * 9 + 4] =
                                m_pNormales[count * 9 + 1] =
                                    (Vec1[2] * Vec2[0] - Vec1[0] * Vec2[2]);
                            m_pNormales[count * 9 + 8] = m_pNormales[count * 9 + 5] =
                                m_pNormales[count * 9 + 2] =
                                    (Vec1[0] * Vec2[1] - Vec1[1] * Vec2[0]);

                            m_pColIndices[count * 3] =
                                cv::saturate_cast<unsigned char>(dpixel1 * 255.0);
                            m_pColIndices[count * 3 + 1] =
                                cv::saturate_cast<unsigned char>(dpixel2 * 255.0);
                            m_pColIndices[count * 3 + 2] =
                                cv::saturate_cast<unsigned char>(dpixel3 * 255.0);
                        }
                        ...
                        if (ito::isFinite<ito::float64>(dpixel1) &&
                            ito::isFinite<ito::float64>(dpixel2) &&
                            ito::isFinite<ito::float64>(dpixel3))
                        {
                            zsum = dpixel1 + dpixel2 + dpixel3;
                            color = zsum / 3.0;
                            isFinite = true;
                        ...
                        if ((fabs(color - dpixel1) < threshold) &&
                            (fabs(color - dpixel2) < threshold) &&
                            (fabs(color - dpixel3) < threshold) &&
                            isFinite /*(fabs(dpixel1) < 1.6e308)*/ && (dpixel1 != invalidValue) &&
                            (dpixel2 != invalidValue) && (dpixel3 != invalidValue))
                        {
#if (USEOMP)
#pragma omp critical
                            ...
                            m_pTriangles[count * 9 + 3] =
                                ((double)(cntX + 1) * m_windowXScale - xshift);
                            m_pTriangles[count * 9 + 4] =
                                ((double)(cntY + 1) * m_windowYScale - yshift);
                            m_pTriangles[count * 9 + 5] = zscale * dpixel2 - zshift;

                            m_pTriangles[count * 9 + 6] =
                                ((double)(cntX + 1) * m_windowXScale - xshift);
                            m_pTriangles[count * 9 + 7] = ((double)(cntY)*m_windowYScale - yshift);
                            m_pTriangles[count * 9 + 8] = zscale * dpixel3 - zshift;
                            ...
                            m_pNormales[count * 9 + 6] = m_pNormales[count * 9 + 3] =
                                m_pNormales[count * 9] = Vec1[1] * Vec2[2] - Vec1[2] * Vec2[1];
                            m_pNormales[count * 9 + 7] = m_pNormales[count * 9 + 4] =
                                m_pNormales[count * 9 + 1] = Vec1[2] * Vec2[0] - Vec1[0] * Vec2[2];
                            m_pNormales[count * 9 + 8] = m_pNormales[count * 9 + 5] =
                                m_pNormales[count * 9 + 2] = Vec1[0] * Vec2[1] - Vec1[1] * Vec2[0];
...
//----------------------------------------------------------------------------------------------------------------------------------
void plotGLWidget::rescaleTriangles(
    const double xscaleing, const double yscaleing, const double zscaleing)
{
    if (m_elementMode == PAINT_POINTS)
    {
        if (!m_pPoints)
...
#pragma omp for schedule(guided)
            ...
            for (ito::int32 n = 0; n < m_NumElements; n++)
            {
                ...
                m_pNormales[n * 9 + 6] = m_pNormales[n * 9 + 3] = m_pNormales[n * 9] =
                    Vec1[1] * Vec2[2] - Vec1[2] * Vec2[1];
                m_pNormales[n * 9 + 7] = m_pNormales[n * 9 + 4] = m_pNormales[n * 9 + 1] =
                    Vec1[2] * Vec2[0] - Vec1[0] * Vec2[2];
                m_pNormales[n * 9 + 8] = m_pNormales[n * 9 + 5] = m_pNormales[n * 9 + 2] =
                    Vec1[0] * Vec2[1] - Vec1[1] * Vec2[0];
            }
#if (USEOMP)
        }
...
void plotGLWidget::refreshPlot(ito::ParamBase* param)
{
    ...
    {
        // check dataObj
        const ito::DataObject* dataObj = param->getVal<const ito::DataObject*>();
        ...
        if (dims > 1)
        {
            ...
            if (dataObj->getType() == ito::tComplex128 || dataObj->getType() == ito::tComplex64)
            {
                if (!m_cmplxState)
                {
                    ((ItomIsoGLWidget*)m_pParent)->enableComplexGUI(true);
                }
            ...
            else
            {
                if (!m_cmplxState)
                {
                    ((ItomIsoGLWidget*)m_pParent)->enableComplexGUI(false);
                }
            ...
            if ((m_axisX.autoScale) || ((int)m_axisX.idx[1] > x1) ||
                ((int)m_axisX.dimIdx != dims - 1))
            {
                m_axisX.idx[0] = 0;
                m_axisX.idx[1] = x1;
            ...
            if ((m_axisY.autoScale) || ((int)m_axisY.idx[1] > y1) ||
                ((int)m_axisY.dimIdx != dims - 2))
            {
                m_axisY.idx[0] = 0;
                m_axisY.idx[1] = y1;
            ...
            m_axisX.phys[0] =
                m_pContentDObj->getPixToPhys(m_axisX.dimIdx, (double)m_axisX.idx[0], test);
            m_axisX.phys[1] =
                m_pContentDObj->getPixToPhys(m_axisX.dimIdx, (double)m_axisX.idx[1], test);

            m_axisY.phys[0] =
                m_pContentDObj->getPixToPhys(m_axisY.dimIdx, (double)m_axisY.idx[0], test);
            m_axisY.phys[1] =
                m_pContentDObj->getPixToPhys(m_axisY.dimIdx, (double)m_axisY.idx[1], test);
            ...
            if (m_objectInfo.show)
            {
                ito::dObjHelper::devValue(
                    m_pContentDObj.data(), 1, m_objectInfo.meanVal, m_objectInfo.divVal, true);
    ...
    if ((param != NULL) && (param->getType() == (ito::Param::PointCloudPtr & ito::paramTypeMask)))
    {
        ...
        case ito::pclXYZRGBNormal:
            ppclXYZRGBNormal = m_pContentPC->toPointXYZRGBNormal().get();
            pclFindMinMax<pcl::PointXYZRGBNormal>(
                ppclXYZRGBNormal, xmin, xmax, ymin, ymax, zmin, zmax);
            break;
...
#else
        retval += ito::RetVal(
            ito::retError, 0, tr("compiled without pointCloud support").toLatin1().data());
#endif // #ifdef USEPCL
    }
    ...
    if (m_pContentDObj != NULL)
    {
        ...
        if (!retval.containsError())
        {
            ...
            if (m_NumElements == 0 || m_forceReplot == true)
            {
                ...
                (m_windowXScale != windowXScaleOld) || (m_windowYScale != windowYScaleOld) ||
                (m_windowZScale != windowZScaleOld))
                rescaleTriangles(
                    m_windowXScale / windowXScaleOld,
                    m_windowYScale / windowYScaleOld,
                    m_windowZScale / windowZScaleOld);
        }
    }
#ifdef USEPCL
    ...
    else
    {
        retval += ito::RetVal(
            ito::retError,
            0,
            tr("DataObject-Container empty and compiled without pointCloud support")
                .toLatin1()
                .data());
    }
#endif // #ifdef USEPCL
    ...
    if (m_pContentDObj == NULL)
#ifdef USEPCL
        if (m_pContentDObj == NULL && m_pContentPC == NULL && m_pContentPM == NULL)
#endif
        {
            m_errorDisplMsg.clear();
...
ito::RetVal plotGLWidget::ResetColors()
{
    ...
    if (m_elementMode == PAINT_POINTS)
    {
        if (m_currentPalette.size() >= 255)
        {
            ...
            {
                m_pColTriangles[4 * count] =
                    ((m_currentPalette[m_pColIndices[count]] & 0xFF0000L) >> 16);
                m_pColTriangles[4 * count + 1] =
                    ((m_currentPalette[m_pColIndices[count]] & 0x00FF00L) >> 8);
                m_pColTriangles[4 * count + 2] =
                    ((m_currentPalette[m_pColIndices[count]] & 0x0000FFL));
    ...
    else
    {
        if (m_currentPalette.size() > 255)
        {
            for (count = 0; count < m_NumElements; count++)
            {
                m_pColTriangles[count * 12] =
                    ((m_currentPalette[m_pColIndices[count * 3]] & 0xFF0000L) >> 16);
                m_pColTriangles[count * 12 + 4] =
                    ((m_currentPalette[m_pColIndices[count * 3 + 1]] & 0xFF0000L) >> 16);
                m_pColTriangles[count * 12 + 8] =
                    ((m_currentPalette[m_pColIndices[count * 3 + 2]] & 0xFF0000L) >> 16);
                m_pColTriangles[count * 12 + 1] =
                    ((m_currentPalette[m_pColIndices[count * 3]] & 0x00FF00L) >> 8);
                m_pColTriangles[count * 12 + 5] =
                    ((m_currentPalette[m_pColIndices[count * 3 + 1]] & 0x00FF00L) >> 8);
                m_pColTriangles[count * 12 + 9] =
                    ((m_currentPalette[m_pColIndices[count * 3 + 2]] & 0x00FF00L) >> 8);
                m_pColTriangles[count * 12 + 2] =
                    ((m_currentPalette[m_pColIndices[count * 3]] & 0x0000FFL));
                m_pColTriangles[count * 12 + 6] =
                    ((m_currentPalette[m_pColIndices[count * 3 + 1]] & 0x0000FFL));
                m_pColTriangles[count * 12 + 10] =
                    ((m_currentPalette[m_pColIndices[count * 3 + 2]] & 0x0000FFL));
                m_pColTriangles[count * 12 + 11] = m_pColTriangles[count * 12 + 7] =
                    m_pColTriangles[count * 12 + 3] = 255;
            }
        }
        else
        {
            /*
                        for (count = 0; count < m_NumElements; count++)
                        {
                            ...
                            m_pColIndices[count * 12 + 11] = m_pColIndices[count * 12 + 7] =
               m_pColIndices[count * 12 + 3] = 255;
                        }
            */
        }
    }
...
void plotGLWidget::threeDAxis(void)
{
    ...
    //    gluProject(m_axisX.phys[0], m_axisY.phys[0], m_axisZ.phys[0], GLModelViewMatrix,
    //    GLProjectionMatrix, GLViewport, &xe[0], &ye[0], &ze[0]); gluProject(m_axisX.phys[0],
    //    m_axisY.phys[0], m_axisZ.phys[0], GLModelViewMatrix, GLProjectionMatrix, GLViewport,
    //    &xe[1], &ye[1], &ze[1]); gluProject(m_axisX.phys[1], m_axisY.phys[1], m_axisZ.phys[1],
    //    GLModelViewMatrix, GLProjectionMatrix, GLViewport, &xe[2], &ye[2], &ze[2]);
    //    gluProject(m_axisX.phys[1], m_axisY.phys[1], m_axisZ.phys[1], GLModelViewMatrix,
    //    GLProjectionMatrix, GLViewport, &xe[3], &ye[3], &ze[3]);
    gluProject(
        xmin,
        ymin,
        zmin,
    ...
    gluProject(
        ...
        &ze[3]);
    gluProject(
        xb, -yb, zmin, GLModelViewMatrix, GLProjectionMatrix, GLViewport, &xe[4], &ye[4], &ze[4]);
    gluProject(
        xb, -yb, zmax, GLModelViewMatrix, GLProjectionMatrix, GLViewport, &xe[5], &ye[5], &ze[5]);
    ...
    */
    ...
    /*
        ...
        std::cout << "dyy: " << (dyy > 0 ? "true" : "false") << "; dyx: " << (dyx > 0 ? "true" :
       "false") <<
                     "; dxy: " << (dxy > 0 ? "true" : "false") << "; dxx: " << (dyx > 0 ? "true" :
       "false") <<
                     "; dzy: " << (dzy > 0 ? "true" : "false") << "; dzx: " << (dzx > 0 ? "true" :
       "false") << "\n";
...
//-----------------------------------------------------------------------------------------------
/**
 *\fn int dreidogl_achse(FilterObject *fo, double x0, double y0, double z0, double x1, double y1,
 *double z1) \brief Zeichnet die Achsenstriche \return error
 */
...
void plotGLWidget::paintLightArrow()
{
    ...
    norm1 = sqrt(
        (position1[0] * position1[0]) + (position1[1] * position1[1]) +
 ...
 *\ingroup 3DOGLFuncsGroup
 */
void plotGLWidget::paintAxisTicksOGL(
    const double x0,
    const double y0,
    const double z0,
    const double x1,
    ...
    const bool write)
{
    ...
    if (write)
    {
        ...
        if (al.unitydigit < al.lastdigit && al.unitydigit != std::numeric_limits<int>::min() &&
            al.lastdigit != std::numeric_limits<int>::max())
        {
        ...
        if (al.unitydigit >= al.lastdigit)
        {
            ...
            else if (al.unitydigit >= -15 && al.unitydigit < 21)
            {
                ...
                if (unit.length())
                {
                    ...
                    char sign[2] = {
                        ("afpn\u00B5m-kMGTPE"[al.unitydigit / 3 + 6]),
                        0}; // Ermittlung der Masseinheit
                    label.append(sign);
        ...
        GLViewport[3] = 2;
        gluProject(
            x0, y0, z0, GLModelViewMatrix, GLProjectionMatrix, GLViewport, &xstart, &ystart, &zpos);
        gluProject(
            x1, y1, z1, GLModelViewMatrix, GLProjectionMatrix, GLViewport, &xend, &yend, &zpos);
    ...
    while (v <= s1)
    {
        ...
        {
            gluProject(
                x0 + a * (x1 - x0) + VorzXAs * ticklength * 3,
                y0 + a * (y1 - y0) + VorzYAs * ticklength * 3,
                z0 + a * (z1 - z0) - VorzZAs * ticklength,
                GLModelViewMatrix,
                GLProjectionMatrix,
                GLViewport,
            ...
            // paintAxisLabelOGL((void*)&al, xpos*(1 + 0.07 * m_windowXScale * fabs(m_axisX.idx[1] -
            // m_axisX.idx[0] + 1.0)),
            //     ypos * (1 + 0.03 * m_windowYScale * fabs(m_axisY.idx[1] - m_axisY.idx[0] + 1.0)),
            //     v);
    ...
    if (write)
    {
        ...
        // al.dx = VorzX * (0.15 * m_windowXScale * fabs(m_axisX.idx[1] - m_axisX.idx[0] + 1.0)) *
        // fabs(sin(phi)); al.dy = VorzY * (0.25 * m_windowYScale * fabs(m_axisY.idx[1] -
        // m_axisY.idx[0] + 1.0)) * fabs(cos(phi));
        //
        // if (al.dx < 0)
 ...
 *\ingroup 3DOGLFuncsGroup
 */
void plotGLWidget::paintAxisLabelOGL(
    const struct AxisLabel& axisLabel, const double x, const double y, const double v)
{
    char buffer[300];
    // long l;
    ...
    else if (axisLabel.unitydigit >= axisLabel.lastdigit)
    {
        _snprintf(
            ...
            sizeof(buffer),
            "%.*f",
            axisLabel.unitydigit - axisLabel.lastdigit,
            v / axisLabel.unity);
    }
    else
    {
        ...
        if (axisLabel.unitydigit == INT_MIN && axisLabel.lastdigit != INT_MAX)
        {
            if (axisLabel.lastdigit >= 0)
            {
                ...
                _snprintf(
                    ...
                    sizeof(buffer),
                    "%.*f",
                    (firstdigit > 0 ? firstdigit : 0) - axisLabel.lastdigit,
                    v);
            else
            {
                double v1, mant;
                ...
                _snprintf(
                    ...
                    "%s%.*fE%d",
                    v < 0 ? "-" : "",
                    firstdigit - axisLabel.lastdigit,
                    mant,
                    expo);
            }
        }
        ...
        else if (firstdigit >= 21 || firstdigit < -18 || axisLabel.unitydigit == INT_MIN)
        {
            ...
            _snprintf(
                ...
                "%s%.0fE%d",
                v < 0 ? "-" : "",
                pow(10.0, log10(v1) - floor(log10(v1))),
                (int)floor(log10(v1)));
        }
        else
        {
            ...
            _snprintf(
                ...
                "%.0f%c",
                pow(10.0, rest),
                "afpn\u00B5m-kMGTPE"[(firstdigit - rest) / 3 + 6]); // Ermittlung der Masseinheit
        }
    }
...
//----------------------------------------------------------------------------------------------------------------------------------
int plotGLWidget::OGLTextOut(
    const char* buffer, double xpos, double ypos, const bool rightAligned, const bool topAligned)
{
    ...
    if (rightAligned)
        xpos -= (1.7 * strlen(buffer) * m_fontsize) / width();
    if (topAligned)
        ypos -= (3.0 * m_fontsize) / height();
...
void plotGLWidget::OGLMakeFont(int size)
{
    ...
    wglUseFontBitmaps(
        GetDC(hwnd), 0, 255, m_myCharBitmapBuffer); // Builds 96 Characters Starting At Character 32
#endif
...
void plotGLWidget::DrawObjectInfo(void)
{
    ...
    if (m_objectInfo.xLength.length())
        OGLTextOut((char*)m_objectInfo.xLength.data(), x0, y0, false, false);

    y0 -= 3.0 * m_fontsize / height();
    if (m_objectInfo.yLength.length())
        OGLTextOut((char*)m_objectInfo.yLength.data(), x0, y0, false, false);

    y0 -= 3.0 * m_fontsize / height();
    if (m_objectInfo.matrix.length())
        OGLTextOut((char*)m_objectInfo.matrix.data(), x0, y0, false, false);
    ...
    if (m_objectInfo.PeakText.length())
        OGLTextOut((char*)m_objectInfo.PeakText.data(), x0, y0, false, false);

    y0 -= 3.0 * m_fontsize / height();
    if (m_objectInfo.MeanText.length())
        OGLTextOut((char*)m_objectInfo.MeanText.data(), x0, y0, false, false);

    y0 -= 3.0 * m_fontsize / height();
    if (m_objectInfo.DevText.length())
        OGLTextOut((char*)m_objectInfo.DevText.data(), x0, y0, false, false);

    return;
}

//----------------------------------------------------------------------------------------------------------------------------------
void plotGLWidget::DrawColorBar(
    const char xPos,
    const char yPos,
    const GLfloat dX,
    const GLfloat dY,
    ...
    const GLfloat zMax)
{
    ...
    OGLTextOut(
        buf,
        x0 + dX + 7.0 / (double)width(),
        y0 - m_fontsize / (double)height() / 2.0,
        false,
        false);
    ...
    OGLTextOut(
        buf,
        x0 + dX + 7.0 / (double)width(),
        y0 + dY - m_fontsize / (double)height() / 2.0,
        false,
        false);
...
void plotGLWidget::DrawTitle(const std::string& myTitle, const int texty, int& yused)
{
    ...
    /*
        ...
        if(strlen(txt))
            OGLTextOut(dd, (char*)txt, (double)x0/width()-0.9, (double)(++i*-2.0*1.5*m_fontsize) /
       height()+0.9);

        tags->ReadTagDef(TAG_COMMENT2,(void *)&txt,"");
        if(strlen(txt))
            OGLTextOut(dd, (char*)txt, (double)x0/this->width()-0.9,
       (double)(++i*-2.0*1.5*m_fontsize)/this->height()+0.9);
...
void plotGLWidget::setColorMap(QString palette)
{
    ...
    {
        src[8 * i] =
            ((unsigned char*)m_currentPalette.data())[4 * m_currentPalette.size() - 4 * i + 2];
        src[8 * i + 1] =
            ((unsigned char*)m_currentPalette.data())[4 * m_currentPalette.size() - 4 * i + 1];
        src[8 * i + 2] =
            ((unsigned char*)m_currentPalette.data())[4 * m_currentPalette.size() - 4 * i];
        src[8 * i + 3] =
            ((unsigned char*)m_currentPalette.data())[4 * m_currentPalette.size() - 4 * i + 3];
        src[8 * i + 4] =
            ((unsigned char*)m_currentPalette.data())[4 * m_currentPalette.size() - 4 * i + 2];
        src[8 * i + 5] =
            ((unsigned char*)m_currentPalette.data())[4 * m_currentPalette.size() - 4 * i + 1];
...
//----------------------------------------------------------------------------------------------------------------------------------
ito::RetVal plotGLWidget::setInterval(
    const Qt::Axis axis, const bool autoCalcLimits, const double minValue, const double maxValue)
{
    /*
        if(m_pContent)
        {
...
void plotGLWidget::setCurrentVisMode(const int mode)
{
    ...
    case PAINT_TRIANG:
        ...
        m_axisZ.show = true;
        if (!m_cmplxState)
            ((ItomIsoGLWidget*)m_pParent)->enableIlluGUI(true);
        break;
    case PAINT_POINTS:
        ...
        m_elementMode = PAINT_POINTS;
        if (!m_cmplxState)
            ((ItomIsoGLWidget*)m_pParent)->enableIlluGUI(false);
        break;
    }
...
inline void plotGLWidget::generateObjectInfoText()
{
    ...
    if (m_axisX.unit.length() && m_axisX.unit.length() < 5)
        sprintf(buf, "Width:  %.4g %s", m_axisX.phys[1] - m_axisX.phys[0], m_axisX.unit.data());
    else
        sprintf(buf, "Width:  %.4g", m_axisX.phys[1] - m_axisX.phys[0]);
    ...
    if (m_pContentDObj)
    {
        ...
        case ito::tInt8:
            sprintf(
                buf,
                "Matrix: %i x %i (%s)",
                m_axisX.idx[1] - m_axisX.idx[0] + 1,
                m_axisY.idx[1] - m_axisY.idx[0] + 1,
                "int8");
            break;
        case ito::tInt16:
            sprintf(
...
void plotGLWidget::rotateLightArrow(const double deltaA, const double deltaB, const double deltaC)
{
    ...
    lighDirAngles[0] = lighDirAngles[0] < -GL_PI
        ? GL_PI + fmod(lighDirAngles[0], GL_PI)
        : (lighDirAngles[0] > GL_PI ? (-GL_PI) + fmod(lighDirAngles[0], GL_PI) : lighDirAngles[0]);
    lighDirAngles[1] = lighDirAngles[1] < -GL_PI
        ? GL_PI + fmod(lighDirAngles[1], GL_PI)
        : (lighDirAngles[1] > GL_PI ? (-GL_PI) + fmod(lighDirAngles[1], GL_PI) : lighDirAngles[1]);
}
//----------------------------------------------------------------------------------------------------------------------------------
void plotGLWidget::rotateView(const double deltaA, const double deltaB, const double deltaC)
{
    ...
    m_RotB += deltaB;
    m_RotC += deltaC;
    m_RotA = m_RotA < -GL_PI ? GL_PI + fmod(m_RotA, GL_PI)
                             : (m_RotA > GL_PI ? (-GL_PI) + fmod(m_RotA, GL_PI) : m_RotA);
    m_RotB = m_RotB < -GL_PI ? GL_PI + fmod(m_RotB, GL_PI)
                             : (m_RotB > GL_PI ? (-GL_PI) + fmod(m_RotB, GL_PI) : m_RotB);
    m_RotC = m_RotC < -GL_PI ? GL_PI + fmod(m_RotC, GL_PI)
                             : (m_RotC > GL_PI ? (-GL_PI) + fmod(m_RotC, GL_PI) : m_RotC);
}
//----------------------------------------------------------------------------------------------------------------------------------
    ...
    const double rotC)
{
    ...
    m_TransY = transY;
    m_TransZ = transZ;
    m_RotA = rotA < -GL_PI ? GL_PI + fmod(rotA, GL_PI)
                           : (rotA > GL_PI ? (-GL_PI) + fmod(rotA, GL_PI) : rotA);
    m_RotB = rotB < -GL_PI ? GL_PI + fmod(rotB, GL_PI)
                           : (rotB > GL_PI ? (-GL_PI) + fmod(rotB, GL_PI) : rotB);
    m_RotC = rotC < -GL_PI ? GL_PI + fmod(rotC, GL_PI)
                           : (rotC > GL_PI ? (-GL_PI) + fmod(rotC, GL_PI) : rotC);
}
//----------------------------------------------------------------------------------------------------------------------------------
