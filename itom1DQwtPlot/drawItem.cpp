...
{
    for (int n = 0; n < m_marker.size(); n++)
        m_marker[n]->setSymbol(new QwtSymbol(QwtSymbol::Diamond,QBrush(((Plot1DWidget*)m_pparent)->m_inverseColor1),
            QPen(QBrush(((Plot1DWidget*)m_pparent)->m_inverseColor1), 3),  QSize(7,7) ));

    if (active == 1)
    {
        if (m_marker.size() >= 1)
            m_marker[0]->setSymbol(new QwtSymbol(QwtSymbol::Rect, QBrush(((Plot1DWidget*)m_pparent)->m_inverseColor1),
                QPen(QBrush(((Plot1DWidget*)m_pparent)->m_inverseColor1), 3),  QSize(7,7) ));
    }
    else if (active == 2)
    {
        if (m_marker.size() >= 2)
            m_marker[1]->setSymbol(new QwtSymbol(QwtSymbol::Rect, QBrush(((Plot1DWidget*)m_pparent)->m_inverseColor1),
                QPen(QBrush(((Plot1DWidget*)m_pparent)->m_inverseColor1), 3),  QSize(7,7) ));
    }
}
//----------------------------------------------------------------------------------------------------------------------------------
...
//            m_marker.remove(n);
    ...
    if (path.elementCount() >= 1)
    {
        ...
        if (((Plot1DWidget*)m_pparent)->m_inverseColor1.isValid())
        {
            marker->setLinePen(QPen(((Plot1DWidget*)m_pparent)->m_inverseColor1));
            marker->setSymbol(new QwtSymbol(QwtSymbol::Diamond,QBrush(((Plot1DWidget*)m_pparent)->m_inverseColor1),
                QPen(QBrush(((Plot1DWidget*)m_pparent)->m_inverseColor1), 3),  QSize(7,7) ));
        }
        else
        {
